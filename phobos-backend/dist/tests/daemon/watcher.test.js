...
describe("watcher", () => {
    ...
    it("processes a file and resolves name conflicts", async () => {
        ...
        const handler = new FileProcessingHandler(watchDir, outputDir, "gdrive:backups");
        ...
        expect(await import("node:fs/promises").then(({ access }) => access(path.join(outputDir, "test_1.jpg")).then(() => true, () => false))).toBe(true);
    });

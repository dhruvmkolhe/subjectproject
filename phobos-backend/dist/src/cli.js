...
import { CleanerError, getFileMetadata, REMOVABLE_GROUPS, sanitizeFile, } from "./services/cleanerService.js";
...
async function promptForConfirmation() {
    ...
    try {
        const answer = await readline.question("Proceed with metadata removal? [y/N] ");
...
export function createCli() {
    ...
    program
        ...
        .option("--show-all-metadata", "Show all metadata (not only removable)", false)
        ...
        try {
            ...
            if (options.showAllMetadata) {
                ...
                const removableEntries = Object.fromEntries(Object.entries(metadata).filter(([key]) => {

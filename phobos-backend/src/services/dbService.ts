...
function mongodbEnabled(): boolean {
...
function getMongoSettings(): {
  ...
  if (!collection) {
    throw new DatabaseError(
      "MONGODB_COLLECTION environment variable is not set",
    );
  }
...
export async function logFileEvent(
  ...
  try {
    ...
    throw new DatabaseError(
      `Cannot log file event: ${(error as Error).message}`,
    );
  }
  ...
  try {
    ...
    throw new DatabaseError(
      `Failed to log file event: ${(error as Error).message}`,
    );
  }
}

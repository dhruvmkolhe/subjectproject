...
export function parseBool(rawValue: string, key: string): boolean {
...
export function loadConfig(): AppConfig {
  ...
  const rcloneRemoteName = getEnv(
    "RCLONE_REMOTE_NAME",
    DEFAULTS.RCLONE_REMOTE_NAME,
    true,
  );
  const rcloneDestPath = getEnv(
    "RCLONE_DEST_PATH",
    DEFAULTS.RCLONE_DEST_PATH,
    true,
  );
  ...
  let mongodbUri: string | null = null;
  let mongodbDatabase: string | null = null;
  if (mongodbEnabled) {
    mongodbUri = getEnv("MONGODB_URI", DEFAULTS.MONGODB_URI, true);
    mongodbDatabase = getEnv(
      "MONGODB_DATABASE",
      DEFAULTS.MONGODB_DATABASE,
      true,
    );
  }

  const mongodbCollection = getEnv(
    "MONGODB_COLLECTION",
    DEFAULTS.MONGODB_COLLECTION,
    true,
  );

...
const cleanedFiles: CleanedFileRecord[] = [];
...
export function createApp(): Express {
  ...
  app.post(
    ...
    async (request: Request, response: Response) => {
      ...
      const remoteBase =
        `${config.rcloneRemoteName}:${config.rcloneDestPath}`.replace(
          /\/+$/,
          "",
        );
      ...
      try {
        ...
        const safeName = path.basename(
          uploadedFile.originalname || path.basename(temporaryFilePath),
        );
        ...
        logger.error(
          `Unexpected error during sanitization: ${(error as Error).message}`,
        );
  ...
  app.post("/backup", async (request: Request, response: Response) => {
    const filePath = request.query.file_path;
    const remote =
      typeof request.query.remote === "string"
    ...
    try {
      ...
      logger.error(
        `Unexpected error during backup: ${(error as Error).message}`,
      );
      sendError(response, 500, "Internal server error");
    }
  });
